...
The staged handoff (laya_apple/handoff.py) runs a forward on this path once a hetero
episode passed its sync guard. The research that measured it is
...
Per predict:
  ...
  5. pop the pool, and copy the outputs out of the backed views with coremltools' names,
     dtypes and shapes (FP16 widened to float32, as coremltools does).
                ...
                if c.keep_output:
                    for name, mode in self.modes.items():
                        if mode != "backed":
                            ...
                            v = _view(c.output.featureValueForName_(name).multiArrayValue(), dtype, shape, strides)
