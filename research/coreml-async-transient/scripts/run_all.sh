#!/bin/sh
# The async transient experiment of research/coreml-async-transient/, unattended: the four runs of
# design.py in order (A r1, PB-ASYNC r1, PB-ASYNC r2, A r2), then analyze.py. It stops no service:
# the machine is prepared beforehand (idle, on AC power, the local LLM server and other GPU/ANE
# services stopped; Core ML E5 cache not cleared).
...
S=research/coreml-async-transient/scripts
D=research/coreml-async-transient/raw
...
uv run python $S/design.py runs | while read -r cell rnd; do
  run "$cell" "$rnd"
done
