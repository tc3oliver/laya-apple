#!/bin/sh
# The whole async-predict screen of research/coreml-async-predict/, unattended. Idle machine on
# AC power, the local LLM server and other GPU/ANE services stopped, Core ML E5 cache not cleared.
#   LAYA_APPLE_CACHE=... HF_HUB_OFFLINE=1 sh research/coreml-async-predict/scripts/run_all.sh
#
# 1. Phase 0 (phase0.py): gates 1-8 of criteria.md on laya, or the screen does not start. Skipped
#    if raw/phase0.json exists with every gate passed.
...
S=research/coreml-async-predict/scripts
D=research/coreml-async-predict/raw
PY="uv run --with pyobjc-framework-CoreML==12.2.2 python"
mkdir -p $D
echo "started $(date '+%Y-%m-%d %H:%M:%S')"
all_pass() {
  [ -e $D/phase0.json ] && uv run python -c "
import json, sys
sys.exit(0 if json.load(open('$D/phase0.json'))['all_pass'] is True else 1)
"
}
if all_pass; then
  echo "skip Phase 0 (raw/phase0.json exists, every gate passed)"
else
  $PY $S/phase0.py --out $D/phase0.json || true  # the test below decides
  all_pass || { echo "Phase 0 failed (raw/phase0.json): the screen does not start" >&2; exit 1; }
fi
