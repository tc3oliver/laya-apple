...
| Same run: multi-question P99 with `auto`, 117.1 ms with the LLM busy against 84.1 ms idle | same, "Decisions" | `results.json`: `cells.auto/{decisions_llm,decisions}.classes.mixed_3q.p99_ms_median` | same | same | same |
