...
- `artifacts build|list [--capabilities]|verify|warm|prune|export|import`, and `fetch` (since 1.6);
...
| `LAYA_APPLE_PREBUILT_REPO` (since 1.6) | The Hugging Face repository `artifacts fetch` reads when `--repo` is not given |
| Prebuilt index, `format: "laya-apple-prebuilt-index"`, `format_version: 1` (since 1.6) | `index.json` at the root of a prebuilt-artifact repository (`laya_apple/prebuilt.py`). Entries may gain fields; a reader refuses other format versions |
