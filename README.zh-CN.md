...
laya-apple 把简短的单问题决策发往 Apple Neural Engine，较长或包含多个问题的决策则留在 MLX GPU
上，两个引擎同时提供服务。从 1.5 开始，laya 和 laya-typed-decisions 改用 Core ML 的异步 API
运行 Neural Engine 请求，避开同步路径长时间持有 GIL 的问题，因此不再拖慢 GPU 返回结果。如果这条路径变慢，
breaker 会把这段 GPU + ANE 同时运行期间剩余的请求送回 1.4 路径。此功能默认开启，无需修改代码。

| 与 1.4 路径相比，一台 Apple M4 Max | laya | laya-typed-decisions |
|---|---:|---:|
| GPU 返回（P50） | 4.28–4.29 → **0.035–0.037 ms** | 8.60 → **0.043 ms** |
| 吞吐量 | **1.042×** | **1.038×** |
| 各 episode P99 的中位数 | **低 0.18–0.19 ms** | **低 0.53 ms** |

- **154 个生产环境验证 episode**（76 个 laya、78 个 typed-decisions，包括突发和长时间 soak）：
  handoff 之后全部停留在异步路径，0 个 mismatch、0 个路由失败、0 个丢失请求、0 次崩溃
  （[`val_tables.md`](research/coreml-adaptive-breaker/val_tables.md)）。
- **恢复是单独测量的，** 因为验证中没有出现变慢状态。在异步路径已经变慢的 12 个 episode 中，
  breaker 在 12 个 episode 中都于 40 ms 内触发，延迟在 164–414 ms 内回到 1.4 路径的水平
...
如需 GPU + ANE 并发服务，请使用 `execution="workers"`，并可从任意线程提交请求：
...
1. **ANE 执行的正确性。** Core ML 导出跑得快，不代表结果一定正确。在我们测试的 Mac 上，普通的
   ...
   （[正确性](#正确性)）。
2. **自动路由。** 路由器在请求运行之前就做出决定，并在 `routing_reason` 中记录原因。
   有负载时，它还会比较两个队列的积压情况。
3. **GPU + ANE 并发服务。** 使用 `execution="workers"` 时，GPU 在 worker 进程中运行，ANE 则有
   自己的 dispatcher。短请求不再排在长请求后面。
4. **自适应 ANE 执行（1.5）。** 每段 GPU + ANE 重叠中，前 64 个 Neural Engine 请求走 1.4 路径，
   之后的请求改用 Core ML 的异步 API。连续三个请求的 prepare 时间超过 0.3 ms 时，这段重叠中剩余的
   请求会退回 1.4 路径
   （[指南](docs/guide.md#adaptive-ane-execution-in-process-ane-the-default-since-15)）。
   `ane_handoff=False` 可关闭此功能。
...
仍然只在显式指定时使用，因为在前一个 bucket（128 个 token）上 ANE 并没有比 MLX 快。

想知道某个请求为什么慢，可以传入一个回调函数（仅限 workers）。每完成一个请求，它都会收到一个
`RequestTrace`：路由器做决定时看到的队列快照、它选择的设备和原因，以及从提交、排队、执行到响应的
monotonic 时间戳（[`docs/api.md`](docs/api.md)）。默认的 `trace=None` 不会记录任何内容。
...
阈值：[`docs/support-matrix.md`](docs/support-matrix.md)。架构：
...
- **回答来自 Laya，而不是 Jev。** 这里测量的只有与上游 Laya 的一致性：792 个请求全部在 FP16
...
**这个 benchmark 早于 1.5：** 它是在 1.3 执行路径上测量的。serve 现在默认使用自适应 ANE 执行，
而这一点尚未在 serve 中测量过。
...
以下是 `laya-apple serve --model laya` 在一台 Apple M4 Max 上跑一次的结果：27B 本地 LLM
（oMLX 上的 `Qwen3.8-27B-oQ4e-mtp`）满载生成，同时以每秒 8 个的速率发送决策请求，其中 80%
是简短的单问题决策（[`benchmarks/serve/`](benchmarks/serve/README.md)，第 2 次运行）。
默认的 `--model auto` 没有测量。
...
- `auto` 的简短决策约有 4% 按设计改走 GPU：当 Neural Engine 的 backlog 等待时间更长时就会这样。
  上面的 P99 已包含这些请求。
- 这个 benchmark 的第 1 次运行没有通过其自身的有效性检查，因此不从中得出任何结论。
...
列车在 100 ms 内没有拿到回答即为晚点。高峰时段会加入突发负载，同时 GPU 上还有长时间运行的后台请求。
...
- 以上是同一台 Apple M4 Max（macOS 26.6.2，`laya-typed-decisions`）上的三次标准运行；
  换成其他 Mac 结果会有所不同。每次运行中，两个轮次对每一趟列车给出的回答都相同。
- 差距大多来自短请求不必再在 GPU 队列里等待，长请求则继续留在 GPU 上运行。
- Benchmark 会先以 headless 模式跑完（首次下载约 800 MB 之后，约需 2–3 分钟），浏览器再回放
  录制的请求 trace；动画本身不计入测量。如果还没有构建 Neural Engine artifact，它只会以仅 GPU
  模式运行，并打印设置命令 `uvx --from "laya-apple[convert]" laya-apple switchyard --setup-ane`。
- 轮次顺序由 seed 决定，所以三次运行都是 `hybrid` 先跑。这些数字不能与下方 v1.0 的表格直接比较：
  两者的速率、测量边界和 workload 都不同。

Workload、方法和原始数据：[`docs/switchyard.md`](docs/switchyard.md) 和
...
- **1.5 的异步路径** 在 golden rows 上必须与 coremltools 的输出完全一致
...
- 一台运行 macOS 26.6.2 的 Apple M4 Max：一条短请求流和一条长请求流，送入同一个
  `Laya(execution="workers")` 实例。这个 benchmark 早于 1.5 的自适应执行。
- 性能提升来自同时使用两个引擎，而不是 ANE 本身的延迟：单个短请求在 ANE 上只是略快一些
  （例如 9.9 ms 对 12.2 ms）。
- 动画先回放一局录制的 [Lane Runner](https://github.com/tc3oliver/laya-playground-apple) 游戏，
  同一个模型分别在两个设备上运行，随后回放 laya-typed-decisions 突发 workload 中的一次突发。
  每个数字的来源都列在 [`docs/media/README.md`](docs/media/README.md)。

方法和原始数据：[`benchmarks/v1.0.md`](benchmarks/v1.0.md)。
...
MLX 和 laya-apple，见 [`benchmarks/v1.0.md`](benchmarks/v1.0.md)。1.5 的检测器回放、恢复实验
和发布验证见
...
- **自适应 ANE 执行只在一台 M4 Max 上测量过。**
  - 154 个验证 episode 都没有出现变慢状态，所以退回后的恢复数据来自同一台机器上另外 12 个
    episode 的实验。
  - 每段 GPU + ANE 重叠的前 64 个 Neural Engine 请求走 1.4 路径。
  - laya-multilingual 的 Neural Engine 在 worker 进程中运行，不使用此功能。
...
- **`laya-apple serve` 的性能只在一种配置下跑过一次：** 一台 M4 Max、一个 LLM（oMLX 上的
  `Qwen3.8-27B-oQ4e-mtp`，以 decode 为主、prompt 很短）、`--model laya`、每秒发送 8 个决策请求
  （[`benchmarks/serve/`](benchmarks/serve/README.md)）。尚未测量的有：其他 LLM 服务器和模型、
  以 prefill 为主的 LLM 负载、其他请求速率、serve 的最大决策吞吐量（第 2 次运行固定以每秒 8 个
  请求发送），以及其他 checkpoint（`laya-typed-decisions`、`--model laya-multilingual`）和
  `--model auto`。
...
- **Switchyard 只在一台 M4 Max 上以固定的轮次顺序运行**（`design.counterbalance` 为 `"none"`，
  所以 `hybrid` 总是先跑）（[`docs/switchyard.md`](docs/switchyard.md)）。
