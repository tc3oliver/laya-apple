...
- **Benchmark disk preflight** (`scripts/bench_preflight.py`). `release_bench.py` and
  ...
  below 200 GiB or the Core ML E5 caches under `~/Library/Caches` exceed 50 GiB, and print
  each cache with its size. Nothing is deleted automatically; `docs/benchmarks.md`
  explains the cache and how to clean it up by hand. No runtime, routing or parity code
  changed.
