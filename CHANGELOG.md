...
- **README serving animation.** `docs/media/heterogeneous-serving.gif` (800×450, 5 fps, 30 s,
  2.3 MB) is the first figure in the README, after the one-line description and before the
  ...
  model on the MLX GPU and the Apple Neural Engine. It then replays one burst of the
  laya-typed-decisions bursty workload, GPU-only against GPU + ANE, from a per-request trace
  of the benchmark's arrival sequence. The P99 values it shows are the published v1.0
  numbers.
